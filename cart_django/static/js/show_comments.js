...
let add_form = (comment_id, path, url_form) => {
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            for (let i = 0; i < data.comment_child_id.length; i++) {
                let comment_id = data.comment_child_id[i];
                let comment_path = data.comment_child_path[i];
                let comment_add = "/comment/comment_add/"+data.product_id+"/"+comment_path;
                document.getElementById('form_cmt'+comment_id).addEventListener('submit', function(event) {
                    console.log("success"+comment_id)
                    event.preventDefault();
                    add_form(comment_id, comment_path, comment_add)
                });
            }
        });
      });
